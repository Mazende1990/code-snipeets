...
namespace ShareX.ImageEffectsLib
{
    ...
    public class DrawImage : ImageEffect
    {
        ...
        public DrawImageSize SizeMode { get; set; }
       ...
       ; set; }
        ...
        public override Bitmap Apply(Bitmap bmp)
        {
            ...
            if (!string.IsNullOrEmpty(imageFilePath) && File.Exists(imageFilePath))
            {
                using (Bitmap bmpWatermark = ImageHelpers.LoadImage(imageFilePath))
                {
                    ...
                    {
                        ApplyWatermarkTransformations(bmpWatermark);
                        Size imageSize = CalculateImageSize(bmpWatermark, bmp);
                        ...
                        DrawWatermark(bmp, bmpWatermark, imageRectangle);
                    }
                }
            }
        ...
        private void ApplyWatermarkTransformations(Bitmap bmpWatermark)
        {
        ...
        private Size CalculateImageSize(Bitmap bmpWatermark, Bitmap bmp)
        {
            return SizeMode switch
            {
                DrawImageSizeMode.AbsoluteSize => ImageHelpers.ApplyAspectRatio(
                    Size.Width == -1 ? bmp.Width : Size.Width,
                    Size.Height == -1 ? bmp.Height : Size.Height,
                    bmpWatermark),
                DrawImageSizeMode.PercentageOfWatermark => ImageHelpers.ApplyAspectRatio(
                    (int)Math.Round(Size.Width / 100f * bmpWatermark.Width),
                    (int)Math.Round(Size.Height / 100f * bmpWatermark.Height),
                    bmpWatermark),
                DrawImageSizeMode.PercentageOfCanvas => ImageHelpers.ApplyAspectRatio(
                    (int)Math.Round(Size.Width / 100f * bmp.Width),
                    (int)Math.Round(Size.Height / 100f * bmp.Height),
                    bmpWatermark),
                _ => bmpWatermark.Size,
            };
        }

        private void DrawWatermark(Bitmap bmp, Bitmap bmpWatermark, Rectangle imageRectangle)
        {
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                else if (Opacity < 100)
                {
                    ...
                    {
                       Matrix matrix = ColorMatrixManager.Alpha(Opacity / 100f);
